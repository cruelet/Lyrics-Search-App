...
async function searchSongs(term) {
  const res = await fetch(`${apiURL}/suggest/${term}`); // put in a missing /
  const data = await res.json();

  showDataUnsafe(data); // changed from console.log to showDataUnsafe
}
...
async function searchSongs(term) {
  ...
  try {
    ...
    result.innerHTML = "<p>Error loading songs.</p>";
  }
  hideLoading();
}
